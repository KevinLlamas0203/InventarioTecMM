...
def update_consumible(id):
    ...
    try:
        ...
        conn.commit()
        detalle = f"Actualizó consumible: {nombre} (ID: {id})"
        registrar_historial(
            accion     = "EDITAR",
            entidad    = "consumible",
            entidad_id = id,
...
def update_stock(id):
    ...
    if not isinstance(cantidad, int) or cantidad < 0:
        return jsonify({"error": "cantidad debe ser un entero no negativo"}), 400
    if operacion in ("sumar", "restar") and cantidad < 1:
        return jsonify({"error": "cantidad debe ser al menos 1 para sumar o restar"}), 400
    ...
    try:
        ...
        if operacion == "restar":
            ...
            """, (cantidad, id))
            accion = "EDITAR"
            detalle = f"Restó {cantidad} del stock del consumible (ID: {id})"
        elif operacion == "set":
            cur.execute("""
                UPDATE consumibles
                SET stock_actual = %s
            ...
            """, (cantidad, id))
            accion = "EDITAR"
            detalle = f"Actualizó stock del consumible a {cantidad} (ID: {id})"
        else:  # sumar
            cur.execute("""
                UPDATE consumibles
            ...
            """, (cantidad, id))
            accion = "EDITAR"
            detalle = f"Sumó {cantidad} al stock del consumible (ID: {id})"
